...
/************************************************ Table A2：Summary Statistics of Personnel Data ****************************************** */
distinct // 查看数据去重个数和行数
count //查看行数


/* Panel A: Reporters */
use "basic panel 04_06.dta", clear
summarize gendre uni_edu party age tenure position qualify


/* Panel B: Division and Copy Editors: 和panel A代码同理 */


/************************************************ Table A3: Summary Statistics of Stayers, Exits, and Entries ****************************************** */

use "basic panel 04_06.dta", clear
// mean 方法可以查看SE，但是论文里的数值明明是汇报的SD
mean gendre uni_edu party age tenure position qualify quantity quality if remainder == 1
mean gendre uni_edu party age tenure position qualify quantity quality if exit == 1
mean gendre uni_edu party age tenure position qualify quantity quality if entry == 1

distinct pid if remainder == 1
distinct pid if exit == 1
distinct pid if entry == 1



/***************************** Table A4: Comparison between Treatment and Control before the Reform****************************************** */
//这个数据感觉用spss 独立样本T检验跑更方便


/***************************** Table A5: Correlation between Quality Score and External Measures****************************************** */
use "external measures.dta", clear 
reg quality articles words investigative feature special propaganda relation plan column , cl(pid) // co_author ex_author 这两个字段他的数据中缺了

/******************************* Table 1：Summary Statistics of Individual Performance Measures ****************************************** */

/* Table 1. Panel A: Summary Statistics of Internal Performance Measures */
use "basic panel 04_06.dta", clear
tabstat articles words quantity quality , statistics(mean sd median min max)


/* Table 1. Panel B: Summary Statistics of Internal Performance Measures */
use "external measures.dta", clear 
sum investigative feature special propaganda relation plan column // co_author ex_author 这两个字段他的数据中缺了
reg quality articles words investigative feature special propaganda relation plan column , cl(pid)
...
set more off //防止 Stata 在输出过多时暂停（默认 Stata 需要按键继续）

preserve //保存当前数据集的状态，以便后续 restore 还原数据

//这里创建了一个 timeindex 变量，将 time 变量（格式类似 200401 表示 2004 年 1 月）转换为一个连续的时间索引（从 1 到 36），用于后续分析
gen timeindex=.    
...
//这里给 timeindex 赋予标签，让其在绘图时能够显示可读的时间格式（如 "Jan. 2004"）。关键时间点 21（2005 年 9 月）被单独标注，这是政策变更时间点。
